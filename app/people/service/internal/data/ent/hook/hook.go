...
func (f PeopleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.PeopleMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PeopleMutation", m)
	}
	return f(ctx, mv)
}
